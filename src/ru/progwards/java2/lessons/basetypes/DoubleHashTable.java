...
public class DoubleHashTable<K extends HashValue, V> implements Iterable<V> {
   ...
   public static void main(String[] args) {
      DoubleHashTable<StringKey, String>strTable = new DoubleHashTable<>();
      for (int i = 0; i < 5; i++) {
         strTable.add(new StringKey(""+i), "Строка - " + i);
      }
      strTable.forEach(System.out::println);

      DoubleHashTable<IntKey, String> intTable = new DoubleHashTable<>();
      for (int i = 0; i < 5; i++) {
         intTable.add(new IntKey(i), "Число: " + i);
      }
      intTable.forEach(System.out::println);
